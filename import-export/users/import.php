...
try {
    ...
    if (!in_array($ext, ['csv', 'xlsx'], true) || $file['size'] > $maxSize || $file['error'] !== UPLOAD_ERR_OK || !is_uploaded_file($file['tmp_name'])) {
        echo json_encode(['status' => 'error', 'error' => 'Invalid upload']);
        exit;
    }

    require_once __DIR__ . '/../../includes/import-export/bootstrap.php';
    $response = Comon_IE_UserImportRunner::processWebRequest($connect, $session, $_POST, $_FILES, $isAdmin, false);
    while (ob_get_level() > 0) {
        ob_end_clean();
    }
    ...
    $base = (isset($lang) && is_array($lang) && !empty($lang['Import failed']))
        ? (string)$lang['Import failed']
        : 'Import failed';
    $detail = trim((string)$e->getMessage());
    if ($detail === '') {
        $detail = (new ReflectionClass($e))->getShortName();
    }
    @error_log('[user_import] ' . $detail . ' @ ' . $e->getFile() . ':' . $e->getLine());
