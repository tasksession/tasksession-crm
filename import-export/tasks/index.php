...
$title = ($lang['Import tasks'] ?? 'Import tasks') . ' | ' . $syatem_title;
...
$taskFields = [
    'Task Title' => $lang['Task Title'] ?? 'Task Title',
    'Description' => $lang['Description'] ?? 'Description',
    'Project ID' => $lang['Project ID'] ?? 'Project ID',
    'Assigned To' => $lang['Assigned To'] ?? 'Assigned To',
    'Start Date' => $lang['Start Date'] ?? 'Start date',
    'Due Date' => $lang['Due Date'] ?? 'Due date',
    'Status' => $lang['Status'] ?? 'Status',
];
$customFields = [];
$cfRes = mysqli_query(
    $connect,
    "SELECT * FROM custom_fields WHERE entity_type = 'task' AND COALESCE(is_disabled, 0) = 0 ORDER BY sort_order ASC, id ASC"
);
if ($cfRes) {
    while ($cfRow = mysqli_fetch_assoc($cfRes)) {
        $taskFields['CustomField_' . $cfRow['id']] = $cfRow['label'];
...
$migrationToolbarH1 = ($lang['Import'] ?? 'Import') . ' — ' . ($lang['Tasks'] ?? 'Tasks');
...
$previewColumns = [
    ...
    ['key' => 'title', 'label' => $lang['Task Title'] ?? 'Task Title'],
    ['key' => 'project_id', 'label' => $lang['Project ID'] ?? 'Project ID'],
                                            ...
                                            <form id="importOptionsForm">
                                                <div class="permission-item d-flex col-gap align-items-center mb-3">
                                                    <div class="checkbox-wrapper-6">
                                                        <input class="tgl tgl-light" id="task_import_dry_run" name="dry_run" type="checkbox" value="1">
                                                        <label class="tgl-btn" for="task_import_dry_run"></label>
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <label for="task_import_dry_run" class="permission-label fw-bold"><?php echo htmlspecialchars($lang['Dry run (no database writes)'] ?? 'Dry run'); ?></label>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
...
window.userImportConfig = {
  ...
  userFields: <?php echo json_encode($taskFields, JSON_UNESCAPED_UNICODE); ?>,
