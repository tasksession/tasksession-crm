...
$title = "Staff Members | ". $syatem_title;
...
{
	if (!has_permission('staff_profile_delete')) {
...
$totalAdminsAndStaff = count($staffs);
$userTypeFilter = '';
$allUsers = $staffs;
...
if (is_array($pickListUsers)) {
	foreach ($pickListUsers as $pu) {
		$uid = (int) ($pu->id ?? 0);
		$ln = function_exists('crm_user_last_name') ? crm_user_last_name($pu) : (string) ($pu->last_name ?? '');
		$fn = trim(($pu->firstName ?? '') . ' ' . $ln);
		...
		$imageHtml = $useImg
			...
			: getUserAvatarHtml($uid, $pu->firstName ?? '', $ln, 32, 32, 'rounded-circle', $pu->firstName ?? '');
		$teamGroupPickableUsers[] = [
			'id' => $uid,
...
$canDeleteMembers = has_permission('staff_profile_delete');
$showMembersBulkToolbar = !$isGroupsView && $canDeleteMembers;
														...
														<div class="icon-container sep">
															<div class="icon-container members-team-tabs">
																<a href="members<?php echo htmlspecialchars($membersTabQuery, ENT_QUOTES, 'UTF-8'); ?>" class="<?php echo !$isGroupsView ? 'active' : ''; ?>">
																	<?php echo ts_icon('user-group'); ?><?php echo htmlspecialchars($lang['Members tab'] ?? 'Members'); ?>
																</a>
																<a href="members<?php echo htmlspecialchars($teamGroupsTabQuery, ENT_QUOTES, 'UTF-8'); ?>" class="<?php echo $isGroupsView ? 'active' : ''; ?>">
																	<?php echo ts_icon('view-grid'); ?><?php echo htmlspecialchars($lang['Team Groups'] ?? 'Team groups'); ?>
																</a>
															</div>
															<a href="user-trash">
								...
								<?php if (!$isGroupsView && $viewType === 'table'): ?>
									...
									<div class="table-responsive scroll-x vh-100">
										<table class="table table-new table-invoice h-100">
											<thead>
												<tr>
													<?php if ($canDeleteMembers) { echo tasksession_list_bulk_checkbox_th_html(); } ?>
											...
											<tbody id="projects-tbl">
												<?php
												if (empty($pagedUsers)) {
													echo '<tr><td colspan="' . ($canDeleteMembers ? '8' : '7') . '">'.$lang['No records Found!'].'</td></tr>';
												} else {
													// Last login map
													$userIds = [];
											...
											<?php
												...
												<div class="col-md-4 col-lg-4 col-xl-3 mb-2 staff user-imgbox img-circle">
													<div class="client-card">
														<?php if ($recentlyRegisteredUser->id != 1 && $canDeleteMembers): ?>
