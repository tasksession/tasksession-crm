...
if (has_permission('project_view_all')) {
    // Show all trashed projects (admin logic)
    $trashProjects = projects::findBySql("SELECT * FROM projects WHERE trash = 1");
    $recentProjects = projects::findBySql("SELECT * FROM projects WHERE trash = 1 ORDER BY start_time DESC");
} else {
    // Only assigned trashed projects (staff logic)
    $id = $session->userId;
    $trashProjects = projects::findBySql("SELECT * FROM projects WHERE find_in_set($id, s_ids) AND trash = 1");
    $recentProjects = projects::findBySql("SELECT * FROM projects WHERE find_in_set($id, s_ids) AND trash = 1 ORDER BY start_time DESC");
}
?>
					...
					<div class="page-content">
						<?php include('../templates/top-header.php'); ?>
									...
									<div class="edit-overview-btn d-block d-md-none">
										 <td class="extra-height">
											<div class="action-toggle" data-bs-toggle="collapse" data-bs-target="#project-menu<?php echo $recentProject->p_id;?>">
												<span class="action-text"><?php echo $lang['Filters']; ?></span><span class="mobile-ellipsis"><?php echo ts_icon('ellipsis'); ?>
										</div>
							...
							<div class="d-none d-md-block">
								...
								<a href="add-new-project" class="btn primary-btn">
