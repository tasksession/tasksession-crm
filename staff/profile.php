...
$profileInvoiceTabVisible = ($invoiceModuleEnabled || $isFreeEdition) && has_permission('milestone_view');
...
$isSelfProfile = ($user_id === $sessionUid);
$targetAccountStatus = (int) $profileUser->accountStatus;

// Direct-link guard: menu hide alone is not enough — enforce view permissions here too.
// Own profile is always allowed; other clients need client_view; other staff/admins need staff_view.
if (!$isSelfProfile) {
	if ($targetAccountStatus === 2) {
		if (!has_permission('client_view')) {
			header('Location: ' . $url . 'staff/index.php');
    exit;
		}
	} elseif ($targetAccountStatus === 1 || $targetAccountStatus === 3 || $user_id === 1) {
...
if ($tab === 'media' && $fileManagementModuleEnabled) {
    $actorStatus = (int)($_SESSION['accountStatus'] ?? 0);
    $isSelfProfile = ((int)$user_id === (int)$session->userId);
    $canViewTargetProfile = false;

    if ($actorStatus === 1) {
        $canViewTargetProfile = true;
    } elseif ($actorStatus === 3) {
        if ($isSelfProfile) {
            $canViewTargetProfile = true;
        } elseif ((int)$profileUser->accountStatus === 2 && function_exists('has_permission') && has_permission('client_view')) {
            $canViewTargetProfile = true;
        } elseif ((int)$profileUser->accountStatus === 3 && function_exists('has_permission') && has_permission('staff_view')) {
            $canViewTargetProfile = true;
        }
    }

    if (!$canViewTargetProfile) {
        $tab = 'overview';
    } else {
        $profileMediaCanView = true;
        if ($actorStatus === 1) {
            $canUploadFiles = true;
            $canEditFiles = true;
            $canDeleteFiles = true;
            ...
            $canMoveFiles = true;
            $canEditOthersFiles = true;
            $canDownloadFiles = true;
            $canCreateFolders = true;
            $canEditFolderPermissions = true;
        } else {
            $canUploadFiles = function_exists('has_permission') && has_permission('project_media_upload');
            $canEditFiles = function_exists('has_permission') && has_permission('project_media_edit');
            $canDeleteFiles = function_exists('has_permission') && has_permission('project_media_delete');
            $canDeleteOthersFiles = function_exists('has_permission') && has_permission('project_media_delete_others');
            $canEditOthersFiles = function_exists('has_permission') && has_permission('project_media_edit_others');
            $canDownloadFiles = function_exists('has_permission') && has_permission('project_media_download');
            $canMoveFiles = ($canDeleteFiles || $canDeleteOthersFiles || $canEditFiles || $canEditOthersFiles);
            $canCreateFolders = $canUploadFiles;
            $canDeleteFolders = ($canDeleteFiles || $canDeleteOthersFiles);
            $canEditFolderPermissions = function_exists('has_permission') && has_permission('project_media_edit_folder_permissions');
        }
    }
}
...
// Last session meta for overview card (used when invoice module is disabled or no permission).
			...
			<div class="row row-eq-height vh-100">
				<?php  include("../templates/sidebar.php"); ?>
					...
					<?php endif; ?>
							...
							<div class="edit-overview-btn  d-block d-md-none">
							 <td class="extra-height">
									...
									<div id="project-menu" class="toggle-action collapse shadow-dept">
										...
										<?php if (has_permission('milestone_view') && $invoiceModuleEnabled && empty($profileInvoiceLocked) && $tab == 'invoice'): ?>
											<li> <a href="add-invoice?return_url=<?php echo urlencode('profile?user_id=' . $user_id . '&tab=invoice'); ?>" class="primary-btn"><?php echo $lang['Add Invoice']; ?></a></li>
										<?php endif; ?>	
										
										<?php if ($tab == 'projects'): ?>
											<li><a href="add-new-project" class="btn primary-btn" target="_blank"><?php echo $lang['Create project']; ?></a></li>
										<?php endif; ?>	
										
										<?php if ($tab == 'tasks'): ?>
											<li><a href="add_task?source=profile&user_id=<?= $user_id ?>" class="btn primary-btn" target="_blank"><?php echo $lang['Add New Task']; ?></a></li>
										<?php endif; ?>	
										...
										<?php if ($tab == 'media' && empty($profileMediaLocked)): ?>
											...
											<li>
												<a href="#" onclick="document.getElementById('uploadBtn').click(); return false;">
													<?php echo ts_icon('upload', 'tasksession-timer-log-menu-ico me-2'); ?>
													<span><?php echo $lang['Upload File'] ?? 'Upload File'; ?></span>
												</a>
											</li>
											<?php endif; ?>
											<?php if (!empty($canCreateFolders)): ?>
											<li>
												<a href="#" data-bs-toggle="modal" data-bs-target="#createFolderModal" onclick="return false;">
													<?php echo ts_icon('folder', 'tasksession-timer-log-menu-ico me-2'); ?>
							...
							<?php if (has_permission('milestone_view') && $invoiceModuleEnabled && empty($profileInvoiceLocked) && $tab == 'invoice'): ?>
							    <div class="d-none d-md-block">
								   <a href="add-invoice?return_url=<?php echo urlencode('profile?user_id=' . $user_id . '&tab=invoice'); ?>" class="primary-btn"><?php echo $lang['Add Invoice']; ?> <?php echo ts_icon('plus', 'w-2'); ?></a>
							</div>
							<?php endif; ?>	
		...
		<?php if(isset($message) && (!empty($message))){echo $message;} ?>
					...
					<div class="center-col max-width-850 pd-30 admin-dashboard">
                       <div class="row counter-align">
                            ...
                            $canViewInvoiceOverview = has_permission('milestone_view') && $invoiceModuleEnabled;
                            ?>
                            <?php if ($projectsModuleEnabled): ?>
							...
							<div class="widget-card stat-spark-card stat-spark-card--invoice-overview widget-card--linked" id="invoice-overview-card">
								...
								<a href="profile?user_id=<?php echo (int)$user_id; ?>&tab=invoice" class="widget-card__stretch-link" aria-label="<?php echo htmlspecialchars($lang['View Invoices'], ENT_QUOTES, 'UTF-8'); ?>"></a>
								  ...
								  <?php endif; ?>    
									...
									<?php
							// Check if current user has permission to view payments
							$canViewFinancials = has_permission('milestone_view') && $invoiceModuleEnabled;
							?>
                            ...
                            <div class="db-box-wrap ">
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <div class="border-btn">
                                        <a href="<?php echo $url; ?>staff/profile?user_id=<?php echo $user_id; ?>&tab=projects">
                                                    ...
                                                    <div class="card project-card" style="position: relative;">
                                                        ...
                                                        <?php echo ts_icon('clock', 'tasksession-timer-log-menu-ico me-2'); ?><?php echo $lang['Overview']; ?>
                                                                ...
                                                                <li>
                                                                 ...
                                                                 <?php if (has_permission('project_edit')): ?><li>
						...
						<?php endif; ?>
                            ...
                            <?php if ($tab == 'invoice'): ?>
                                ...
                                <?php elseif (!has_permission('milestone_view') || !$invoiceModuleEnabled): ?>
                                    <?php 
                                    ...
                                    <div class="row vh-100">
                                        <div class="col-12">
                                            ...
                                            <div class="row  pd-30" id="invoice-grid">
                                                ...
                                                if ((int)$profileUser->accountStatus == 3 || (int)$profileUser->accountStatus == 1) { // Staff or Admin
                                                    $milestonesQuery = "SELECT m.*, m.currency, m.is_recurring, m.recurring_frequency, p.project_title FROM milestones m LEFT JOIN projects p ON m.p_id = p.p_id WHERE $whereClause ORDER BY m.deadline DESC";
                                                            ...
                                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                                ...
                                                                <div class="dropdown">
                                                                    ...
                                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenu<?php echo $milestone['id']; ?>">
                                                                        ...
                                                                        <?php if ($milestone['status'] != 1 && has_permission('milestone_update')): // Only show "Mark as Paid" for unpaid invoices ?>
                                                                        <li>
                                                                        ...
                                                                        <?php endif; ?>
                                                                        <?php if ($milestone['status'] != 1 && $milestone['status'] != 2 && has_permission('milestone_update')): // Only show "Cancel Invoice" for unpaid invoices ?>
                                                                        <li>
                                                                            <form method="post" action="#" class="d-inline">
                                                                                <input type="hidden" name="invoice_id" value="<?php echo $milestone['id']; ?>">
                                                                        ...
                                                                        <?php endif; ?>
                                                                        <?php if ($milestone['status'] != 1 && $milestone['status'] != 2): // Only show "Copy payment link" for unpaid invoices ?>
                                                                        <li>
                                                                        ...
                                                                        <?php if ($milestone['status'] != 1 && has_permission('milestone_delete')): // Only show "Delete" for unpaid invoices ?>
                                                                        <li>
                            ...
                            <?php elseif ($fileManagementModuleEnabled): ?>
                                <?php
                            ...
                            <?php if ($tab == 'tasks'): ?>
                                ...
                                if ($profileUser->accountStatus == 3 || $profileUser->accountStatus == 1) {
                                    ...
                                    $tasksQuery = "SELECT t.*, 
                                        ...
                                        // Add search filter with prepared statement
                                        $searchPattern = '';
                                        if (!empty($searchQuery)) {
                                            $searchPattern = "%{$searchQuery}%";
                                            $tasksQuery .= " AND (t.title LIKE ? OR t.description LIKE ?)";
                                        }
                                        
                                        $tasksQuery .= " ORDER BY t.created_at DESC";
                                        
                                        // Execute with prepared statement (only for search)
                                        $stmt = $connect->prepare($tasksQuery);
                                        if ($stmt === false) {
                                            error_log("Tasks query prepare failed: " . $connect->error . " | Query: " . $tasksQuery);
                                            $tasks = [];
                                        } else {
                                            ...
                                            if (!$stmt->execute()) {
                                                error_log("Tasks query execute failed: " . $stmt->error);
                                                $tasks = [];
                                            } else {
                                                $result = $stmt->get_result();
                                                $tasks = [];
                                ...
                                <div class="board-wrap">
                                    <div class="board d-flex">
                                        <?php 
                                        // Always show all columns, rename Done when a special status filter is active
                                        $activeFilterName = '';
                                        if ($statusFilter && in_array($statusFilter, ['overdue','due_today','task_pro','on_time'])) {
                                            switch ($statusFilter) {
                                                case 'overdue': $activeFilterName = 'Overdue Tasks'; break;
                                                case 'due_today': $activeFilterName = 'Due Today'; break;
                                                case 'task_pro': $activeFilterName = 'Task Pro'; break;
                                                case 'on_time': $activeFilterName = 'On Time'; break;
                                            }
                                        }
                                        ...
                                        ?>
                                            <div class="board-column" 
                                                ...
                                                <h2 class="h6 d-flex align-items-center">
                                                    ...
                                                    <span class="column-title" data-column="<?= $status ?>"><?php echo ($activeFilterName && $status==='done') ? $activeFilterName : $statusLabels[$status]; ?></span>
                                                ...
                                                <?php if (!empty($columnTasks)): ?>
                                                    ...
                                                    <div class="load-more-container" id="load-more-<?= $status ?>" style="display: block;">
                                                        <button class="btn primary-btn w-100 load-more-btn" 
                                                                data-status="<?= $status ?>" 
                                                                data-page="2">
...
<link rel="stylesheet" href="../vendor/google/gdrive/assets/css/file-sharing.css">
...
header('location:profile?user_id='.$user_id.'&tab=invoice&message=notupdated'); 
	...
	// Handle "Mark as Paid"
	if (isset($_POST['mark_as_paid']) && isset($_POST['invoice_id']) && has_permission('milestone_update')) {
	...
	if (isset($_POST['cancel_invoice']) && isset($_POST['invoice_id']) && has_permission('milestone_update')) {
		$invoice_id = (int)$_POST['invoice_id'];
		$milestone = milestone::findById($invoice_id);
		if ($milestone) {
...
if (is_file($__aiCtx)) { require_once $__aiCtx; if (function_exists('ai_contextual_emit')) { ai_contextual_emit(['type' => 'client', 'id' => (int) $user_id]); } } ?>
