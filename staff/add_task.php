...
// Initialize message early so POST handler can set it and it persists
$message = "";
...
if (isset($_POST['add-task'])) {
    ...
    // Staff-side project scope validation to prevent crafted POST access.
    $postedProjectId = (int) ($task->project_id ?? 0);
    $postedProject = null;
    if ((int)($_SESSION['accountStatus'] ?? 0) === 3 && $postedProjectId > 0) {
        require_once("../includes/projects.php");
        $postedProject = projects::findByProjectId($postedProjectId);
        if (!$postedProject) {
            $validationErrors[] = 'Invalid project selected.';
        } else {
            $isAssignedToProject = strpos((string) $postedProject->s_ids, (string) $session->userId) !== false;
            if (!has_permission('project_view_all') && !$isAssignedToProject) {
                $validationErrors[] = 'You are not authorized to create tasks in this project.';
            }
        }
    }

    if ((int) ($_SESSION['accountStatus'] ?? 0) === 3 && !empty($assignedIds)) {
        if ($canAssignMembers) {
            $allowedAssigneeIds = [];
            $assigneeRows = User::findBySql('SELECT id FROM users WHERE (accountStatus = 3 OR accountStatus = 1) AND status = 0');
            foreach ($assigneeRows as $assigneeRow) {
                $allowedAssigneeIds[(int) $assigneeRow->id] = true;
            }
            foreach ($assignedIds as $assigneeId) {
                if (empty($allowedAssigneeIds[$assigneeId])) {
                    $validationErrors[] = 'One or more selected assignees are not valid.';
                    break;
                }
            }
        } elseif ($postedProjectId > 0 && !empty($postedProject)) {
            $projectAssigneeIds = array_map('intval', array_filter(explode(',', (string) $postedProject->s_ids)));
            foreach ($assignedIds as $assigneeId) {
                if (!in_array($assigneeId, $projectAssigneeIds, true)) {
                    $validationErrors[] = 'You can only assign team members who are on this project.';
                    break;
                }
            }
        }
...
if($_SESSION['accountStatus'] == 3){
    // Check if staff has task creation permission
    if (!has_permission('task_create')) {
        redirectTo($url."staff/dashboard?message=permission_denied");
    }
}
...
// $message initialized above (do not reset after POST)
...
// Assign dropdown: all staff when internal task OR staff may assign any member; else project team only.
if (!$project || $canAssignMembers) {
    ...
    foreach ($assignedStaffIds as $staffId) {
        $staffId = (int) $staffId;
        if ($staffId > 0 && $staffId != (int) $project->c_id) {
...
if ($canAssignMembers) {
    foreach ($allProjects as $proj) {
        $projectStaffMap[(int) $proj->p_id] = $staffList;
    }
} else {
    foreach ($allProjects as $proj) {
        $staffArr = [];
        $assignedStaffIds = explode(',', (string) $proj->s_ids);
        foreach ($assignedStaffIds as $staffId) {
            $staffUser = User::findById($staffId);
            if ($staffUser && ($staffUser->accountStatus == 3 || $staffUser->accountStatus == 1)) {
                $fullName = $staffUser->firstName;
        ...
        $projectStaffMap[(int) $proj->p_id] = $staffArr;
    }
}
// For internal tasks (no project), show all staff
