...
// Auth before header (keeps session headers clean)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_delete_action']) && $_POST['bulk_delete_action'] === 'delete') {
    require_once("../includes/task.php");
    if (has_permission('task_delete') && isset($_POST['bulk_delete_tasks']) && is_array($_POST['bulk_delete_tasks'])) {
...
$allTasksCanUpdateStatus = has_permission('task_status_update');
$allTasksCanEdit = has_permission('task_edit');
$allTasksCanDuplicate = has_permission('task_duplicate');
...
function buildAllTasksListUrl(array $overrides = []) {
    global $statusFilter, $searchQuery, $showMyTasks, $showArchive, $sortOrder, $internalFilter, $startDateFilter, $endDateFilter, $kanbanShowAllTasksTab;
...
function staffTasksAssignedFilter($tableAlias = '') {
    global $id;
    $col = $tableAlias !== '' ? $tableAlias . '.assigned_to' : 'assigned_to';
    return "FIND_IN_SET($id, $col) > 0";
}

function staffShouldFilterAssignedTasks() {
    global $showMyTasks, $showArchive, $id;
    return staffAllTasksScopeSql('', $showMyTasks, $showArchive, (int) $id) !== null;
}

function staffTasksScopeFilterSql($tableAlias = '') {
    global $showMyTasks, $showArchive, $id;
    return staffAllTasksScopeSql($tableAlias, $showMyTasks, $showArchive, (int) $id);
}
...
if ($kanbanCanViewAllTasks) {
    $totalCountQuery = "SELECT COUNT(*) as total FROM tasks WHERE " . $activeTaskFilter;
    ...
    $createdScope = Task::kanbanCreatedTasksWhere((int) $id, 'tasks');
    $totalCountQuery = "SELECT COUNT(*) as total FROM tasks WHERE ($activeTaskFilter) AND $createdScope";
    $totalCountResult = $database->query($totalCountQuery);
    $totalCountData = $database->fetchArray($totalCountResult);
...
$myTasksCountQuery = "SELECT COUNT(*) as total FROM tasks WHERE ($activeTaskFilter) AND " . staffTasksAssignedFilter();
...
$archivedCountQuery = "SELECT COUNT(*) as total FROM tasks WHERE is_archived = 1";
$archivedScope = staffAllTasksScopeSql('', false, true, (int) $id);
if ($archivedScope !== null) {
    $archivedCountQuery .= ' AND ' . $archivedScope;
}
...
$scopeSql = staffTasksScopeFilterSql('');
if ($scopeSql !== null) {
    $countWhere[] = $scopeSql;
}
...
$scopeSql = staffTasksScopeFilterSql('t');
if ($scopeSql !== null) {
    $where[] = $scopeSql;
}
if (count($where) > 0) {
                        ...
                        <div class="search">
                                    ...
                                    if (is_readable($aiMenuPartial)) {
                                        try {
                                            ...
                                            error_log('staff all-tasks ai-menu: ' . $e->getMessage());
                                        }
                                    }
                                    unset($aiMenuId);
                                    $showMobileAddTask = has_permission('task_create');
                          ...
                          <?php endif; ?>
                            ...
                            <div class="table-responsive scroll-x vh-100">
                                <table class="table table-new projectspage all-tasks-table" id="tasks-table">
                                    <?php
                                    // Always show checkbox column; disable when staff cannot delete.
                                    $allTasksShowCheckbox = true;
                                    $allTasksCheckboxDisabled = !$kanbanCanDelete;
                                    echo allTasksTableTheadHtml($lang, true, (bool) $allTasksCheckboxDisabled);
                                    ?>
