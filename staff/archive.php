	...
	<div class="page-container">
		<div class="container-fluid">
								...
								<div class="project-tabs-header">
									<div class="scrollable-tabs-container d-flex col-gap-40 col-gap-40-sep">
										...
										if (has_permission('project_view_all')) {
											...
											// Only assigned archived projects (staff logic)
											$id = $session->userId;
											$archiveProjects = projects::findBySql("SELECT * FROM projects WHERE find_in_set($id, s_ids) AND archive = 1 AND trash != 1");
											$recentProjects = projects::findBySql("
												SELECT * FROM projects 
												WHERE find_in_set($id, s_ids) AND archive = 1 AND trash != 1 $searchCondition $statusCondition
												ORDER BY start_time DESC 
												LIMIT $start_from, $limit
											");
											$recentProjects_page = projects::findBySql("
										...
										?>       <div class="main-heading"><h1><?php echo $lang['Archive Projects']; ?> <span>(<?php echo count($archiveProjects); ?>)</span></h1></div>		
											<div class="icon-container sep">
												<?php 
												// Get project counts based on permissions
												if (has_permission('project_view_all')) {
													$projects = projects::findBySql("SELECT * FROM projects WHERE archive = 0 AND trash != 1");
													$trashProjects = projects::findBySql("SELECT * FROM projects WHERE trash = 1");
												} else {
													$id = $session->userId;
													$projects = projects::findBySql("SELECT * FROM projects WHERE find_in_set($id, s_ids) AND archive = 0 AND trash != 1");
													$trashProjects = projects::findBySql("SELECT * FROM projects WHERE find_in_set($id, s_ids) AND trash = 1");
												}
												?>
		        <a href="projects">
					...
					<?php echo $lang['Trash Projects']; ?> (<?php echo count($trashProjects); ?>) </a>
                   </div>
				</div>   
			 </div> 
										<div class="search">
			...
			<div class="clearfix"></div>
					<?php 
					// Pagination variables are already set above
					...
					?>
							<div class="row">
								<?php if(isset($message) && (!empty($message))){echo $message;} ?>
										...
										<?php // $recentProjects is already set above based on permissions ?>
											...
											<div class="table-responsive scroll-x vh-100">
												<table class="table table-new projectspage">
													<thead>
														<tr>
															<?php if ($canDeleteProject) { echo tasksession_list_bulk_checkbox_th_html(); } ?>
															<th width="26%">
													...
													<tbody id="projects-tbl">
														...
														if($recentProjects == NULL){
														  ...
														  foreach($recentProjects as $recentProject){ ?>
															<tr>
																<?php if ($canDeleteProject) { echo tasksession_list_bulk_checkbox_td_html($recentProject->p_id, $row_counter); } ?>
											...
											<div class="row pagination-box">
												<div class="col-md-6 resilts-txt">
													<?php echo $lang['Showing']; ?> <span class="start_val"><?php echo $start_from + 1;?></span>
														<?php echo $lang['to']; ?> <span class="end_val"><?php echo min($start_from + $limit, $total_records); ?></span>
															<?php echo $lang['of']; ?>
