...
function client_bulk_payment_json_response(array $payload, int $status = 200, bool $flushDeferredEmails = false): void
{
...
try {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        client_bulk_payment_json_response(['success' => false, 'message' => 'Invalid request method'], 405);
    }

    csrf_require_for_request('json');

    if (!$session->isLoggedIn() || (int) $_SESSION['accountStatus'] !== 2) {
        client_bulk_payment_json_response(['success' => false, 'message' => 'Unauthorized'], 401);
    }

    if (empty($_POST['bulk_pay'])) {
        client_bulk_payment_json_response(['success' => false, 'message' => 'Invalid bulk payment request'], 400);
    }

    $clientId = (int) $session->userId;
    $invoiceId = (int) ($_POST['invoice_id'] ?? 0);
    $gatewayId = isset($_POST['gateway']) ? (string) $_POST['gateway'] : 'stripe';

    $adminSettings = settings::findById(1);
    if (!$adminSettings) {
        client_bulk_payment_json_response(['success' => false, 'message' => 'Payment settings unavailable'], 500);
    }
    ...
    $result = client_bulk_payment_process_one(
        $gatewayId,
        $clientId,
        $invoiceId,
        $paymentPayload,
    ...
    client_bulk_payment_json_response($result, !empty($result['success']) ? 200 : 422, $flushEmails);
} catch (Throwable $e) {
    error_log('client-bulk-payment-process.php: ' . $e->getMessage());
    client_bulk_payment_json_response([
        'success' => false,
        'message' => 'Unable to process invoice payment',
    ], 500);
}
