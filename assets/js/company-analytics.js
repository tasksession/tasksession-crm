// Company Analytics JavaScript ÃƒÂ‚Ã‚Â— company profile sales statistics
// Based on analytics.js but for user-specific data

// Check if Chart.js is loaded
if (typeof Chart === 'undefined') {
    console.error('Chart.js is not loaded. Please ensure Chart.js is included before company-analytics.js');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (earningsCanvas) {
    ...
    try {
      companyEarningsLineChart = new Chart(ctx, {
    type: 'line',
    plugins: [{
  ...
  window.earningsLineChart = companyEarningsLineChart;
           } catch (error) {
             console.error('Error creating company earnings chart:', error);
           }
         } else {
...
function refreshInvoiceOverview(startDate, endDate) {
    ...
    xhr.onload = function() {
        ...
        try {
            ...
            // Sparks + % only â€” amounts/counts stay locked to Sales stats (user-data).
...
function updateCompanyRangeTotals(startDate, endDate) {
    ...
    if (companyId) {
        fetch(`${companyDataAjaxUrl()}?company_id=${companyId}&start_date=${startDate}&end_date=${endDate}&currency=${encodeURIComponent(currency)}`)
            .then(response => response.json())
            .then(data => {
                
                if (data.success) {
                    // Update chart data
                    if (companyEarningsLineChart) {
                        companyEarningsLineChart.data.labels = data.monthLabels || data.labels || [];
                        companyEarningsLineChart.data.datasets[0].data = data.monthlyEarnings || data.paid || [];
                        companyEarningsLineChart.data.datasets[1].data = data.monthlyUnpaid || data.unpaid || [];
                        if (companyEarningsLineChart.data.datasets[2]) {
                          companyEarningsLineChart.data.datasets[2].data = data.monthlyCancelled || data.cancelled || [];
                        }
                        companyEarningsLineChart.update();
                    }
                    
                    // Update totals
                    if (data.totalPaid !== undefined) {
                        window.companyTotalPaid = data.totalPaid;
                    } else if (data.paid_total !== undefined) {
                        window.companyTotalPaid = data.paid_total;
                    }
                    if (data.totalUnpaid !== undefined) {
                        window.companyTotalUnpaid = data.totalUnpaid;
                    } else if (data.unpaid_total !== undefined) {
                        window.companyTotalUnpaid = data.unpaid_total;
                    }
                    window.companyTotalCancelled = data.cancelled_total || 0;
                    window.companyTotalTax = data.sales_tax_total || 0;
                    window.companyPaidPctChange = data.paid_pct_change;
                    window.companyVsLabel = data.vs_label;
...
function getCompanyIdFromUrl() {
    if (window.companyProfileId) {
        return String(window.companyProfileId);
    }
...
function selectQuickRange(range) {
  ...
  updateCompanyRangeTotals(formatDate(start), formatDate(end));
...
function selectCustomRange() {
  ...
  if (startInput && endInput) {
    ...
    if (start && end) {
      updateCompanyRangeTotals(start, end);
      var customRangeDropdown = document.getElementById('customRangeDropdown');
      if (customRangeDropdown) {
        customRangeDropdown.innerText = `${start} ÃƒÂƒÃ‚Â¢ÃƒÂ‚Ã‚Â€ÃƒÂ‚Ã‚Â“ ${end}`;
      }
    }
  }
}
...
function selectCurrency(currency) {
    ...
    if (window.currentDateRange && window.currentDateRange.start && window.currentDateRange.end) {
        updateCompanyRangeTotals(window.currentDateRange.start, window.currentDateRange.end);
    } else {
        // If no date range set, use current year
        const today = new Date();
        const startOfYear = new Date(today.getFullYear(), 0, 1);
        const endOfYear = new Date(today.getFullYear(), 11, 31);
        updateCompanyRangeTotals(formatDate(startOfYear), formatDate(endOfYear));
    }
}
...
function updateCompanyTotals() {
    updateSalesStatsSummary({
        paid_total: window.companyTotalPaid || 0,
        unpaid_total: window.companyTotalUnpaid || 0,
        cancelled_total: window.companyTotalCancelled || 0,
        sales_tax_total: window.companyTotalTax || 0,
        currency_symbol: window.currentCurrencySymbol || getCurrencySymbol(window.selectedCurrency || 'all'),
        paid_pct_change: window.companyPaidPctChange,
        vs_label: window.companyVsLabel
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
  function toggleDatasetFromSummary(item) {
    var chart = companyEarningsLineChart || window.earningsLineChart;
    if (!chart || !item) {
      return;
    }
