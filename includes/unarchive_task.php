...
try {
    ...
    $canUnarchive = ($userType === 1) || ($userType === 3 && function_exists('has_permission') && has_permission('task_edit'));
    if (!$canUnarchive) {
        echo json_encode(['status' => 'error', 'error' => 'Not authorized to unarchive this task']);
        exit;
    }
    ...
    if ($task->unarchive()) {
        ...
        echo json_encode(['status' => 'error', 'error' => 'Failed to unarchive task']);
    }
} catch (Exception $e) {
