...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    redirectTo($url . "admin/task?projectId={$projId}&message=columnRenamed");
}
...
if ($_SESSION['accountStatus'] == 3) redirectTo($url . "staff/index.php");

// Get current user info (bootstrap already loaded $userb)
$user = $userb ?? User::findById((int)$id);
$username = $user ? $user->firstName : '';
$email = $user ? $user->email : '';

// Redirect to kanban.php if no projectId in GET
if (!isset($_GET['projectId']) || empty($_GET['projectId'])) {
    redirectTo($url . "admin/kanban");
}
...
// ----------------------------------------------------------------------------
// ④— LOAD PROJECT & RELATED DATA (sidebar include loads project + stats)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// ⑥— PROJECT STAFF FOR TASK CARDS (from sidebar batch load)
// ----------------------------------------------------------------------------
$projectStaff = [];
if (!empty($staffMembers)) {
    foreach ($staffMembers as $staffUser) {
        if ($staffUser && ((int)$staffUser->accountStatus === 3 || (int)$staffUser->accountStatus === 1)) {
            $projectStaff[(int)$staffUser->id] = $staffUser;
        }
    }
}
    ...
    <div class="container-fluid">
        <div class="row row-eq-height">
            ...
            <div class="page-content">
                ...
                <div class="row" id="project-layout-row">
                    <div class="container-fluid">
                        <div class="row">
                                ...
                                <div class="modal fade" id="columnRenameModal" tabindex="-1">
                                    <div class="modal-dialog modal-sm">
                                        <form id="columnRenameForm" 
                                              action="<?= $url ?>admin/task?projectId=<?= $projectId ?>" 
                                              method="post" 
                                              class="modal-content">
