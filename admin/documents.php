...
if (isset($_SESSION['accountStatus']) && (int) $_SESSION['accountStatus'] === 2) {
...
if (isset($_POST['save_note'])) {
    ...
    if (empty($content)) {
        $message = "<div class='alert alert-danger'>Note content cannot be empty.</div>";
        ...
        if (empty($access['can_edit'])) {
            ...
            $encrypted_content = encryptString($content);
            $sql = "UPDATE private_notes SET title='{$database->escapeValue($title)}', content='{$database->escapeValue($encrypted_content)}', updated_at=NOW() WHERE id={$note_id}";
            $result = $database->query($sql);
            $message = $result ? "<div class='alert alert-success'>Note updated!</div>" : "<div class='alert alert-danger'>Error updating note.</div>";
        }
    } else {
        ...
        $encrypted_content = encryptString($content);
        $sql = "INSERT INTO private_notes (user_id, title, content, color, created_at, updated_at) VALUES ({$user_id}, '{$database->escapeValue($title)}', '{$database->escapeValue($encrypted_content)}', '{$color_to_assign}', NOW(), NOW())";
        $result = $database->query($sql);
        if ($result) {
...
if (isset($_GET['delete'])) {
    ...
    if (!empty($deleteAccess['is_owner'])) {
        ...
        if ($currentTab === 'trash') {
            ...
            $database->query($sql);
            $message = "<div class='alert alert-success'>Note deleted!</div>";
        } else {
...
if (isset($_GET['msg']) && $_GET['msg'] === 'created_with_color' && isset($_GET['color']) && in_array($_GET['color'], $allowedColors)) {
    $message = "<div class='alert alert-success'>Note created with color <strong>" . htmlspecialchars(ucfirst($_GET['color'])) . "</strong>!</div>";
}
...
if ($tabFilter !== 'trash') {
    ...
    $sharedMePn = (in_array($scopeFilter, $allowedScopeFilters, true) && $scopeFilter === 'shared_with_me')
        ...
        : '';
    $seenPn = [];
    $sqlPnRec = "SELECT pn.*, s.permission AS access_permission
    ...
    $resRec = $database->query($sqlPnRec);
    while ($resRec && ($pr = $database->fetchArray($resRec))) {
        $pid = (int) ($pr['id'] ?? 0);
        if ($pid <= 0 || isset($seenPn[$pid])) {
            continue;
        }
        $seenPn[$pid] = true;
        if (!empty($pr['content'])) {
...
$pnRecCountRes = $database->query(
    'SELECT COUNT(DISTINCT pn.id) AS c FROM profile_notes pn
    INNER JOIN profile_note_shares s ON s.profile_note_id = pn.id AND s.shared_with_user_id = ' . (int) $user_id . '
    WHERE (pn.is_archived = 0 OR pn.is_archived IS NULL)'
);
$pnRecRow = $pnRecCountRes ? $database->fetchArray($pnRecCountRes) : null;
$total_notes += (int) ($pnRecRow['c'] ?? 0);
...
$row_archive_count = $database->fetchArray($result_archive_count);
$archive_notes = (int)($row_archive_count['total'] ?? 0);
$pnArchRecRes = $database->query(
    'SELECT COUNT(DISTINCT pn.id) AS c FROM profile_notes pn
    INNER JOIN profile_note_shares s ON s.profile_note_id = pn.id AND s.shared_with_user_id = ' . (int) $user_id . '
    WHERE pn.is_archived = 1'
);
$pnArchRec = $pnArchRecRes ? $database->fetchArray($pnArchRecRes) : null;
$archive_notes += (int) ($pnArchRec['c'] ?? 0);
...
$row_trash_count = $database->fetchArray($result_trash_count);
                    ...
                    <div class="col-md-12 project-tabs">
						<div class="row">
                                                ...
                                                <div class="input-group">
                                                   <input type="text" id="task-search" name="search" class="form-control" placeholder="<?php echo $lang['Search notes...']; ?>" value="<?php echo htmlspecialchars($searchQuery ?? ''); ?>">
                                                    ...
                                                    <?php if(isset($searchQuery) && !empty($searchQuery)): ?>
                                                        <a href="?<?php echo $statusFilter ? 'status='.htmlspecialchars($statusFilter) : ''; ?>" 
                                                           class="cross">
							...
							$is_editing = (isset($edit_note['id']) && $edit_note['id']) || (isset($_GET['note_id']) && intval($_GET['note_id']))
								|| (!empty($edit_note['_is_profile_shared']));
                            ...
                            <button type="button" class="btn primary-btn d-none d-md-inline-flex align-items-center" id="openShareNoteBtn" <?php echo empty($edit_note['id']) ? 'disabled' : ''; ?>>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($notes as $note): ?>
                                            <?php
                                                ...
                                                $pnCardId = $isProfShare
                                                    ? (int) ($note['profile_note_id'] ?? 0)
                                                    : (int) ($note['id'] ?? 0);
                                                if ($isProfShare) {
                                                    $noteHref = 'documents?profile_shared_note=' . $pnCardId;
                                                } else {
                                                    $noteHref = 'documents?note_id=' . (int) ($note['id'] ?? 0);
                                                }
                                            ...
                                            <li class="list-group-item note-card <?php echo $colorClass; ?><?php if ($isRowActive) {
                                                ...
                                                <a href="<?php echo htmlspecialchars($noteHref, ENT_QUOTES, 'UTF-8'); ?>" class="note-card-link">
                                                    <div>
													...
													<div class="d-flex doc-date">                                     
                                                     <?php echo date('Y-m-d', strtotime($note['created_at'])); ?>
                                                      ...
                                                      <?php
                                                        ...
                                                        $isOwnerView = !$isProfShare && ($noteOwnerId === (int) $user_id);
                                                      ...
                                                      <div class="mt-2 note-card-share-meta">
                                                          ...
                                                          <div class="<?php echo $hasTeamShare ? 'mt-2 pt-2 br-top' : ''; ?>">
                                                              ...
                                                              <div class="badge note-card-public-link-wrap<?php echo ((string)($note['public_share_permission'] ?? 'view')) === 'edit' ? ' note-card-public-link-wrap--edit' : ''; ?> note-card-public-link-trigger" role="button" tabindex="0" data-note-id="<?php echo (int)$note['id']; ?>" title="Open public link settings">
                                                ...
                                                <div class="dropdown note-card-menu" style="position: absolute; top: 10px; right: 15px;">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenu<?php echo $note['id']; ?>">
                            ...
                            <div class="col-lg-9 px-0 bg-white private-note notes-editor-loading admin-docs-editor" id="privateNoteEditorPane">
                                ...
                                <div>
                                    <form method="post" action="" id="noteForm">
                                        ...
                                        <input type="hidden" name="profile_shared_note_id" value="<?php echo !empty($edit_note['profile_note_id']) && !empty($edit_note['_is_profile_shared']) ? (int) $edit_note['profile_note_id'] : ''; ?>">
                                        ...
                                        <div class="form-group mb-2">
                                            <textarea name="note_content" class="form-control notes-textarea" placeholder="Type your notes here..." <?php echo (!$current_can_edit && !isset($_GET['new'])) ? 'readonly' : ''; ?>><?php echo isset($edit_note['content']) ? htmlspecialchars($edit_note['content']) : ''; ?></textarea>
                                            <textarea name="title" class="note-title-under-editor" placeholder="Title" rows="1" required <?php echo (!$current_can_edit && !isset($_GET['new'])) ? 'readonly' : ''; ?>><?php echo isset($edit_note['title']) ? htmlspecialchars($edit_note['title']) : ''; ?></textarea>
                                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function mountTitleBetweenToolbarAndBody() {
        ...
        // Auto-grow title textarea for long headings on mobile/desktop.
