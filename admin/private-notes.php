...
$title = "Private Notes | " . $syatem_title;
...
if (!isset($user_id) || empty($user_id)) {
    die('User not logged in or userId not set.');
}
...
if (isset($_POST['save_note'])) {
    ...
    if (empty($content)) {
        ...
        $sql = "UPDATE private_notes SET title='{$database->escapeValue($title)}', content='{$database->escapeValue($encrypted_content)}', updated_at=NOW() WHERE id={$note_id} AND user_id={$user_id}";
							...
							<div class="project-tabs-header">
								<div class="scrollable-tabs-container d-flex col-gap-40 col-gap-40-sep">
									<div class="main-heading d-flex align-items-center justify-content-between">
										<div><h1> <?php echo $lang['Notes']; ?><span> (<?php echo $total_notes; ?>)</span></h1></div>
									</div>
									<?php
										$showBack = false;
										if (
										...
										<div class="search">
											...
											<div class="edit-overview-btn">
													 <td class="extra-height">
															<div class="action-toggle" data-toggle="collapse" data-target="#color-menu">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($notes as $note): ?>
                                            ...
                                            <li class="list-group-item note-card <?php echo $colorClass; ?><?php if (isset($edit_note['id']) && $edit_note['id'] == $note['id']) echo ' active'; ?>" style="position: relative;">
                                                <a href="private-notes.php?note_id=<?php echo $note['id']; ?>" class="note-card-link">
                                                    <div>
                                ...
                                <div>
                                    <form method="post" action="" id="noteForm">
                                        ...
                                        <div class="form-group mb-2">
                                            <textarea name="note_content" class="form-control notes-textarea" placeholder="Type your notes here..."><?php echo isset($edit_note['content']) ? htmlspecialchars($edit_note['content']) : ''; ?></textarea>
                                            <textarea name="title" class="note-title-under-editor" placeholder="Title" rows="1" required><?php echo isset($edit_note['title']) ? htmlspecialchars($edit_note['title']) : ''; ?></textarea>
                                        </div>
