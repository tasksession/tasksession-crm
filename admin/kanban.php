...
// Handle bulk delete tasks (same logic as projects.php)
        ...
        if (!empty($taskIds)) {
            ...
            foreach ($taskIds as $taskId) {
                ...
                if ($task->delete()) {
                    ...
                    // Log the deletion
                    error_log("Task ID $taskId deleted by user ID " . $session->userId . " at " . date('Y-m-d H:i:s'));
                } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['column_key'], $_POST['column_name'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Insert new column name for this user
...
function buildFilterUrl($baseUrl, $additionalParams = []) {
    global $statusFilter, $searchQuery, $startDateFilter, $endDateFilter, $showMyTasks, $showArchive, $sortOrder, $internalFilter;
...
if ($showArchive) {
    ...
    if (!has_permission('task_view_all')) {
        $allTasks = array_filter($allTasks, function($task) use ($id) {
            if (empty($task->assigned_to)) return false;
            $assignedIds = array_map('trim', explode(',', $task->assigned_to));
            return in_array($id, $assignedIds);
        });
    }
...
if ($internalFilter) {
    ...
    $allTasks = array_filter($allTasks, function($task) use ($statusFilter, $searchQuery, $startDateFilter, $endDateFilter, $showMyTasks, $showArchive, $id, $kanbanRecurringFilterMap) {
        ...
        if ($showMyTasks && !$showArchive) {
            if (empty($task->assigned_to)) {
                ...
                $assignedIds = array_map('trim', explode(',', $task->assigned_to));
                $assignedMatch = in_array($id, $assignedIds);
            }
        }
        
        return $statusMatch && $searchMatch && $dateMatch && $assignedMatch;
    });
}
}
...
$kanbanTabCounts = Task::kanbanTabCounts((int)$id, true, false);
...
// Handle messages (toast only — no inline alerts)
...
if ($toast_flash === null && !empty($session->message)) {
    $toastType = 'success';
    $msgText = (string)$session->message;
                                ...
                                <div class="board-wrap">
                                    <div class="board d-flex full-width">
                                        ...
                                        ?>
                                            <div class="board-column" 
                                                ...
                                                <h2 class="h6 d-flex align-items-center">
                                                    ...
                                                    <span class="column-title" data-column="<?= $key ?>"><?= $cfg['label'] ?></span>
                                                                    ...
                                                                    <div class="d-flex justify-content-between align-items-start">
                                                                        ...
                                                                        <div class="dropdown">
                                                                            ...
                                                                            <ul class="dropdown-menu dropdown-menu-end">
                                                                                ...
                                                                                <?php if (isset($task->project_id) && $task->project_id > 0): ?>
                                                                                <li>
                                        ...
                                        <div class="board-column add-column-col">
										 <h2 class="h6 d-flex align-items-center"><span class="column-title"  id="addColumnBtn"><?php echo $lang['+ Add New Column']; ?></span></h2>
									   </div>
                                    </div>
                                </div>
                            </div>
...
<!-- Column Rename Modal -->
				...
				<div id="kanban-lock-btn" title="Lock/Unlock Kanban">
				  <svg id="kanban-lock-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4">
					<path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V7.5A4.5 4.5 0 0 0 7.5 7.5v3m9 0A2.25 2.25 0 0 1 21 12.75v4.5A2.25 2.25 0 0 1 18.75 19.5h-13.5A2.25 2.25 0 0 1 3 17.25v-4.5A2.25 2.25 0 0 1 5.25 10.5h13.5zm-6-7.5v3" />
				  </svg>
				</div>
