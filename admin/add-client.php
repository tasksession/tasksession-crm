...
if(isset($_POST['add-client']))
{
    ...
    if($flag == 0)
    {
        ...
        if ($invoiceModuleEnabled) {
            ...
            // Keep a consistent default when invoice module is disabled.
        ...
        else
        {
            ...
            if ($stmt) {
                $stmt->bind_param("isssissssssssssssssssssss", 
                    $id, $Projects_ids, $hashedPassword, $email, $accountStatus, $firstName, $title, $address, 
                    $phone, $website, $teams_id, $fb, $regDate, $type_status, $last_seen, $session_status, 
                    $status, $note, $city, $state, $zip, $country, $user_language, $currency, $assigned_team
                );
                if ($stmt->execute()) {
                ...
                if (empty($templateHTML)) {
                    ...
                    <body>
                        ...
                        <ul>
                            <li><strong>Email:</strong> {USER_LOGIN_EMAIL}</li>
                            <li><strong>Password:</strong> {USER_LOGIN_PASSWORD}</li>
                        </ul>
                        <p>You can login to your account at: <a href='{DASHBOARD_URL}'>{DASHBOARD_URL}</a></p>
                ...
                $emailStatus = $emailHelper->getEmailStatus();
                error_log("EmailHelper Status: " . json_encode($emailStatus));
				  ...
				  <?php if(isset($message) && (!empty($message))){ echo $message; } ?>
					<div class="add-client center-col">
                        ...
                        <div class="form-group submit-box" id="permissions-submit-box" style="display: none;">
                          ...
                          <div><button type="submit" name="add-client" id="create-client-btn" class="primary-btn"><?php echo $lang["Create Account"]; ?></button></div>
						</div>
                      </div>
                   </form>
                </div>
