...
$calendarHttpHost = isset($_SERVER['HTTP_HOST']) ? (string)$_SERVER['HTTP_HOST'] : '';
$calendarDebugHostMatch = stripos($calendarHttpHost, 'demo-dark') !== false;
$calendarExplicitOff = isset($_GET['calendar_debug']) && (string)$_GET['calendar_debug'] === '0';
$calendarEarlyShutdown = !$calendarExplicitOff && (
    $calendarDebugHostMatch || (isset($_GET['calendar_debug']) && (string)$_GET['calendar_debug'] === '1')
);
/** Blank white screen: catch fatal errors and flush buffers so a visible box appears (demo-dark or ?calendar_debug=1). */
if ($calendarEarlyShutdown) {
    ...
    register_shutdown_function(function () {
        ...
        $msg = htmlspecialchars((string)$err['message'], ENT_QUOTES, 'UTF-8');
        $file = htmlspecialchars((string)$err['file'], ENT_QUOTES, 'UTF-8');
        $line = (int)$err['line'];
        echo '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Calendar — PHP fatal</title></head><body style="margin:0;background:#1c1917;color:#fecaca;font:15px/1.5 system-ui,sans-serif;">';
        echo '<div style="padding:14px 18px;background:#450a0a;border-bottom:3px solid #ef4444;color:#fff;"><strong>Calendar — PHP fatal</strong> (shutdown)</div>';
        echo '<div style="padding:18px;max-width:960px;"><p><strong>Message:</strong><br><code style="word-break:break-word;">' . $msg . '</code></p>';
        echo '<p><strong>File:</strong> <code>' . $file . '</code></p><p><strong>Line:</strong> ' . $line . '</p>';
        echo '<p style="color:#a8a29e;margin-top:24px;">Tip: fix the error above, or open with <code>?calendar_debug=0</code> on demo-dark to hide this handler.</p></div></body></html>';
    });
}
...
require_once $base . '/includes/task.php';
require_once $base . '/includes/calendar_event.php';
...
if ($calendarReady) {
    ...
    $scheduledTasks = CalendarEvent::listScheduledTasksByMonth($userId, $renderStartDate, $renderEndDate, $showMyTasks);
    foreach ($scheduledTasks as $taskRow) {
        if ($showMyTasks) {
            $assignedRaw = isset($taskRow['assigned_to']) ? (string)$taskRow['assigned_to'] : '';
            $assignedList = array_filter(array_map('trim', explode(',', $assignedRaw)));
            if (!in_array((string)$userId, $assignedList, true)) {
                continue;
            }
        }
    ...
    $events = CalendarEvent::listEventsByMonth($userId, $renderStartDate, $renderEndDate, $showMyTasks);
...
if ($showMyTasks) {
    $allTasksForCalendar = array_filter($allTasksForCalendar, function ($task) use ($userId) {
        if (!isset($task->assigned_to) || $task->assigned_to === null || $task->assigned_to === '') {
            return false;
        }
        $assigned = array_map('trim', explode(',', (string)$task->assigned_to));
        return in_array((string)$userId, $assigned, true);
    });
}
...
$calendarFilterQs = !$showMyTasks ? 'all_tasks=1&' : '';
                        ...
                        <div class="row">
                            ...
                            <div class="search">
                                ...
                                <form method="GET" action="" class="search-form" id="searchForm">
                                    <?php if (!$showMyTasks): ?><input type="hidden" name="all_tasks" value="1"><?php endif; ?>
                        ...
                        <div class="edit-overview-btn">
                            ...
                            <div id="project-menu-calendar" class="toggle-action shadow-dept collapse">
                                <ul>
                                    <li class="d-block d-md-none">
                                        <a href="add_task?source=calendar" class="primary-btn-a">
                                    ...
                                    <li class="<?php echo (!$startDateFilter && !$endDateFilter && $taskDateMode === 'all') ? 'active' : ''; ?>" data-status="today">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars(date('Y-m')); ?>&date=<?php echo htmlspecialchars(date('Y-m-d')); ?>">
                                            <span><?php echo $lang['Today'] ?? 'Today'; ?></span>
                                        </a>
                                    </li>
                                    <li class="<?php echo $taskDateMode === 'all' ? 'active' : ''; ?>" data-status="all">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars($selectedMonth); ?>&date=<?php echo htmlspecialchars($selectedDate); ?>&task_date_mode=all">
                                            <span><?php echo $lang['All Schedule'] ?? 'All schedule'; ?></span>
                                        </a>
                                    </li>
                                    <li class="<?php echo $taskDateMode === 'due' ? 'active' : ''; ?>" data-status="due_date">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars($selectedMonth); ?>&date=<?php echo htmlspecialchars($selectedDate); ?>&task_date_mode=due">
                                            <span><?php echo $lang['Due Date'] ?? 'Due date'; ?></span>
                                        </a>
                                    </li>
                                    <li class="<?php echo $taskDateMode === 'start' ? 'active' : ''; ?>" data-status="start_date">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars($selectedMonth); ?>&date=<?php echo htmlspecialchars($selectedDate); ?>&task_date_mode=start">
                                    ...
                                    <li class="<?php echo $sortOrder === 'asc' ? 'active' : ''; ?>" data-status="ascending">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars($selectedMonth); ?>&date=<?php echo htmlspecialchars($selectedDate); ?>&task_date_mode=<?php echo htmlspecialchars($taskDateMode); ?>&sort_order=asc<?php echo $startDateFilter ? '&start_date=' . htmlspecialchars($startDateFilter) : ''; ?><?php echo $endDateFilter ? '&end_date=' . htmlspecialchars($endDateFilter) : ''; ?>">
                                            <span>
                                    ...
                                    <li class="<?php echo $sortOrder === 'desc' ? 'active' : ''; ?>" data-status="descending">
                                        <a href="?<?php echo $calendarFilterQs; ?>month=<?php echo htmlspecialchars($selectedMonth); ?>&date=<?php echo htmlspecialchars($selectedDate); ?>&task_date_mode=<?php echo htmlspecialchars($taskDateMode); ?>&sort_order=desc<?php echo $startDateFilter ? '&start_date=' . htmlspecialchars($startDateFilter) : ''; ?><?php echo $endDateFilter ? '&end_date=' . htmlspecialchars($endDateFilter) : ''; ?>">
                                            <span>
                            ...
                            <div class="d-none d-md-block calendar-add-new-md-wrap">
                                <div class="edit-overview-btn calendar-add-new-collapse">
                                    <div id="primary-btn" class="action-toggle primary-btn collapsed calendar-add-new-toggle" data-bs-toggle="collapse" data-bs-target="#calendarAddNewMenu" aria-expanded="false" role="button" tabindex="0">
                                    ...
                                    <div id="calendarAddNewMenu" class="toggle-action collapse shadow-dept">
                                        <ul>
                                            <li><a class="d-flex align-items-center" href="add_task?source=calendar"><?php echo ts_icon('tasks', 'tasksession-timer-log-menu-ico me-2'); ?><?php echo $lang['Add Task'] ?? 'Add Task'; ?></a></li>
...
<?php include $base . '/templates/calendar-event-modal.php'; ?>
<?php include $base . '/templates/event-sidebar.php'; ?>
<?php
$__calCssPath = $base . '/assets/css/calendar.css';
...
<script>
    window.calendarPageData = {
        ...
        allTasks: <?php echo $showMyTasks ? '0' : '1'; ?>,
...
<?php
/** Relative to /admin/calendar.php — avoids broken calendar when DB site_url root ≠ real install path (e.g. subfolder). */
$__calendarJsPath = $base . '/assets/js/calendar.js';
$__calendarJsVer = (string)(@filemtime($__calendarJsPath) ?: time());
$__calendarJsSrcRel = '../assets/js/calendar.js?v=' . $__calendarJsVer;
$__stickyHeadersJsPath = $base . '/assets/js/kanban-sticky-headers.js';
...
include $base . '/templates/main-footer.php';
